...
class ClientDtoTest {
    ...
    void testSearchSuccess() throws ApiException {
        ...
        when(clientApi.search(ClientSearchType.NAME, "test", 0, 5)).thenReturn(pojoPage);

        ClientSearchForm searchForm = new ClientSearchForm();
        searchForm.setType(ClientSearchType.NAME);
        searchForm.setQuery("test");
        searchForm.setPage(0);
        searchForm.setSize(5);

        Page<ClientData> result =
                clientDto.search(searchForm);

        assertEquals(1, result.getTotalElements());
        assertEquals("test client", result.getContent().get(0).getName());

        verify(clientApi).search(ClientSearchType.NAME, "test", 0, 5);
    }
}
